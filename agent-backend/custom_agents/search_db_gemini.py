...
class GeminiSearchAgent:
       ...
       Bạn là chuyên gia BĐS. Nhiệm vụ: Query dữ liệu chính xác từ Database.
        ...
        1. Xử lý GIÁ TIỀN (Quan trọng):
           ...
           * LOGIC NÂNG CAO:
           ...
           Chú ý phân biệt kĩ price, max_price và min_price, khi không nói rõ thì chú ý là price thôi, vì nếu gán sai sẽ gặp vấn đề rất lớn

        2. Xử lý ĐỊA ĐIỂM:
        - Hãy trích xuất TỈNH/THÀNH PHỐ vào trường `province`. VD: "Đất nền ở Đà Nẵng" -> province=["Đà Nẵng"].
        - Trích xuất QUẬN/HUYỆN vào trường `district`. VD: "Nhà ở Quận 1" -> district=["1"]. "Nhà ở huyện hoài đức" -> district=["Hoài Đức"]
        - Trích xuất PHƯỜNG/XÃ vào trường `ward`. 

        
        3. CHIẾN THUẬT:
           - Ưu tiên dùng `search_posts_strict` cho các câu hỏi cụ thể. Hạn chế tối đa gọi hàm `search_posts` vì nó làm sai yêu cầu của người dùng

           - Cố gắng map estate_type về các loại chuẩn: "nhà mặt phố", "nhà phố", "nhà riêng", "nhà trọ", "phòng trọ", "chung cư", "tập thể", "căn hộ",
            "biệt thự", "liền kề", "đất biệt thự", "đất", "đất mặt phố", "đất riêng", "đất trang trại", "kho xưởng", "thuê kho", "văn phòng", "cửa hàng", "nhà đất khác"
            - Ví dụ: đất mặt phố -> ["đất mặt phố"] chứ không phải ["đất","nhà mặt phố"]
        """
    ...
    def _sanitize_args(self, args: dict) -> dict:
        ...
        # 2. Xử lý List String (Province, District, Ward) + Ép kiểu str()
        # Fix lỗi: Google trả về RepeatedScalarContainer không phải list thuần -> json dump lỗi
        for list_field in ['province', 'district', 'ward', 'estate_type']:
            if list_field in new_args:
                raw = new_args[list_field]
                # Chuyển đổi mọi thứ thành List của String Python thuần túy
                safe_list = []
                if isinstance(raw, str):
                    safe_list = [str(raw)]
                else:
                    # Duyệt qua iterator để ép kiểu
                    safe_list = [str(item) for item in raw]
                
                new_args[list_field] = safe_list


        # 3. Xử lý province và district
        if 'province' in new_args:
            raw = new_args['province']
            if isinstance(raw, str):
                new_args['province'] = [raw]
        ...
        for p_field in ['price', 'min_price', 'max_price', 'square', 'front_face', 'front_road']:
