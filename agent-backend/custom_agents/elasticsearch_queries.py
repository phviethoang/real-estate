...
def search_posts_strict(
    ...
    if district:
        must_clauses.append({"terms": {"district": district}})
    if ward:
...
# --- ĐOẠN CODE SỬA (BẮT ĐẦU) ---
    ...
    if min_price is not None:
        price_range_query["gte"] = min_price
    if max_price is not None:
        price_range_query["lte"] = max_price
...
if __name__ == "__main__":
    result = search_posts_strict(estate_type=["nhà riêng"], is_latest_posted=True, district=["Hai Bà Trưng"], no_bedrooms=2, no_floors=2, max_price=10000000000.0)
